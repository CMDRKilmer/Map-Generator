...
class MapWidget(QWidget):
    ...
    def _draw_roads(self, painter: QPainter):
        ...
        for hc, td in self.terrain_data.items():
            ...
            cx, cy = self.hex_grid.hex_center(hc, self.hex_size)
            for nh in hc.neighbors():
                if nh not in self.terrain_data or not self.terrain_data[nh].road:
                    continue
                edge = (
                    hc.q * 100000 + hc.r,
                    nh.q * 100000 + nh.r,
    ...
    def _draw_shipping_routes(self, painter: QPainter):
        ...
        for hc, td in self.terrain_data.items():
            ...
            cx, cy = self.hex_grid.hex_center(hc, self.hex_size)
            for nh in hc.neighbors():
                if nh not in self.terrain_data or not self.terrain_data[nh].shipping:
                    continue
                edge = (
                    hc.q * 100000 + hc.r,
                    nh.q * 100000 + nh.r,
